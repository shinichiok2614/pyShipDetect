...
while True:
    ...
    for i, (*xyxy, conf, cls) in enumerate(boxes, start=1):
        ...
        if label == 'boat':
            ...
            cv2.putText(frame, text, org, font, font_scale, (0, 255, 0), thickness)

            # Tính kích thước chữ 'boat ' để đặt số sát bên phải
            (text_width, _), _ = cv2.getTextSize(text, font, font_scale, thickness)

            # Vẽ số màu vàng ngay bên phải chữ 'boat '
            number_org = (x1 + text_width, y1 - 10)
            cv2.putText(frame, f"#{i}", number_org, font, font_scale, (0, 255, 255), thickness, cv2.LINE_AA)

            # Phần thumbnail như cũ
